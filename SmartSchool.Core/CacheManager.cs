...
namespace SmartSchool
{
    ...
    public abstract class CacheManager<T>
    {
        ...
        private AutoResetEvent _Loading = new AutoResetEvent(true);
        ...
        private bool _SyncAllBackgroundRunning = false;
        private bool _SyncAllBackgroundAgain = false;
        private List<ItemUpdatedEventArgs> _UntriggeredItemUpdatedEvent = new List<ItemUpdatedEventArgs>();
        private bool _UntriggeredItemLoadedEvent = false;
        ...
        public CacheManager()
        {
            ...
            {
                List<ItemUpdatedEventArgs> list = new List<ItemUpdatedEventArgs>();
                lock (_UntriggeredItemUpdatedEvent)
                {
                    list.AddRange(_UntriggeredItemUpdatedEvent);
                    _UntriggeredItemUpdatedEvent.Clear();
                }
                if (ItemUpdated != null)
                {
                    foreach (var itemUpdatedEventArgs in list)
                    {
                        ItemUpdated(this, itemUpdatedEventArgs);
                    }
                }
                if (_UntriggeredItemLoadedEvent)
                {
                    _UntriggeredItemLoadedEvent = false;
                    if (ItemLoaded != null)
        ...
        protected virtual void OnItemUpdated(ItemUpdatedEventArgs itemUpdatedEventArgs)
        {
            if (ItemUpdated != null)
            {
                if (Thread.CurrentThread.IsBackground)
                {
                    lock (_UntriggeredItemUpdatedEvent)
                    {
                        _UntriggeredItemUpdatedEvent.Add(itemUpdatedEventArgs);
                    }
                }
                else
                    ItemUpdated(this, itemUpdatedEventArgs);
        ...
        protected virtual void OnItemLoaded(EventArgs eventArgs)
        {
            if (ItemLoaded != null)
            {
                if (Thread.CurrentThread.IsBackground)
                {
                    _UntriggeredItemLoadedEvent = true;
                }
                else
